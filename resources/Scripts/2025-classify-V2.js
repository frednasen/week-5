...
function writeToSheet(classifiedTerms) {
  const sheet = SpreadsheetApp.openByUrl(SHEET_URL);
  ...
  resultsSheet.clear();
  const data = classifiedTerms.map((classification, index) => [classification]);
  resultsSheet.getRange(1, 1, data.length, 1).setValues(data);
}
