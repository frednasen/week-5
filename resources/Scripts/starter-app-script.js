...
function main() {
  try {
    Logger.log("Running script with search terms limit: 40");
...
function processTab(ss, tabName, headers, query, dataProcessor) {
  try {
    ...
    // Run the query
    Logger.log("Executing query for " + tabName);
    ...
    if (data.length > 0) {
      Logger.log("Writing " + data.length + " rows to sheet " + tabName);
...
function calculateSearchTermsMetrics(rows) {
  ...
  Logger.log("Total search terms processed: " + rowCount);
